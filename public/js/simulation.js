// game Object ********************************************************
//*********************************************************************
// JSON replacer function for selective serialization
function replacer(key, value) {
  if (key === "parts" || key === "vertices" || key === "parent") {
    return null;
  }
  return value;
}
...
socket.on("playerPositions", (players) => {
  otherPlayers = Object.values(players).filter((player) => player.id !== socket.id);

  Object.values(players).forEach((player) => {
    if (!m2.playerBodies[player.id] && socket.id !== player.id) {
      m2.spawn(player.id);
    }
  });

  Object.values(players).forEach((player) => {
    m2.updatePosition(player.id, player.pl.pos, player.pl.yOff, player.pl.isCrouch);
  });
});
let otherBullets = [];
socket.on("updateBullets", (bullets) => {
  otherBullets = bullets;
  console.log(bullets);
});

socket.on("removeBody", (playerId) => {
  m2.deleteBody(playerId);
});
...
function serializeBullets() {
  ...
  for(let i = 0; i < bullet.length; i++) {
    let bulletVertices = [];
    for(let j = 0; j < bullet[i].vertices.length; j++) {
      bulletVertices.push({x:bullet[i].vertices[j].x, y:bullet[i].vertices[j].y});
    }
    vertices.push(bulletVertices);
  }
  return bullet.map((b) => ({
    type: b.bulletType,
...
function stonkLoop() {
  ...
  const data = {
    ...
    pl: {
      ...
      bodyGradient: m.bodyGradient,
      fillStyle: m.fillStyle,
      flipLegs: m.flipLegs,
      stepSize: m.stepSize,
      legLength1: m.legLength1,
      legLength2: m.legLength2,
      velocity: player === undefined ? { x: 0, y: 0 } : player.velocity,
      isCrouch: m.crouch,
  ...
  socket.emit("bulletChange", {bullets: serializeBullets(), id: socket.id});
}
window.addEventListener("beforeunload", () => {
  socket.emit("requestBodyDeletionServerSide", socket.id);
  socket.disconnect();
});

...
const m = {
  ...
  setFillColors() {
    ...
    m.fillColorDark = `hsl(${m.color.hue},${m.color.sat}%,${
      m.color.light - 25
    }%)`;
      ...
      for (let i = 0, len = 5; i < len; i++) {
        setTimeout(function () {
          ...
          simulation.makeTextLog(
            `simulation.amplitude <span class='color-symbol'>=</span> 0.${
              len - i - 1
            }`,
            swapPeriod
          );
  ...
  setMaxHealth() {
    m.maxHealth =
      m.baseHealth +
      tech.extraMaxHealth +
      2.22 * tech.isFallingDamage +
      4 * tech.isFlipFlop * tech.isFlipFlopOn * tech.isFlipFlopHealth;
    document.getElementById("health-bg").style.width = `${Math.floor(
      300 * m.maxHealth
    )}px`;
    simulation.makeTextLog(
      `<span class='color-var'>m</span>.<span class='color-h'>maxHealth</span> <span class='color-symbol'>=</span> ${m.maxHealth.toFixed(
        2
      )}`
    );
    if (m.health > m.maxHealth) m.health = m.maxHealth;
  ...
  defense() {
    ...
    if (tech.isSpeedHarm && player.speed > 0.1)
      dmg *= 1 - Math.min(player.speed * 0.0165, 0.66);
    if (tech.isHarmReduce && input.field && m.fieldCDcycle < m.cycle)
      dmg *= 0.25;
  ...
  damage(dmg) {
    ...
    if (tech.isDroneOnDamage && bullet.length < 150) {
      ...
      const len =
        (Math.min((dmg - 0.06 * Math.random()) * 40, 40) /
    ...
    if (tech.isEnergyHealth) {
      ...
      if (m.energy < 0 || isNaN(m.energy)) {
        ...
        if (
          ...
          if (m.immuneCycle < m.cycle + 300) m.immuneCycle = m.cycle + 300; //disable m.immuneCycle bonus seconds
          simulation.wipe = function () {
            //set wipe to have trails
  ...
  otherCalcLeg(cycle_offset, offset, other) {
    other.hip.x = 12 + offset;
    other.hip.y = 24 + offset;
    //stepSize goes to zero if Vx is zero or not on ground (make m transition cleaner)
    other.stepSize =
      0.8 * other.stepSize +
      0.2 * (7 * Math.sqrt(Math.min(9, Math.abs(other.Vx))) * other.onGround);
    //changes to stepsize are smoothed by adding only a percent of the new value each cycle
    const stepAngle = 0.034 * other.walk_cycle + cycle_offset;
    other.foot.x = 2.2 * other.stepSize * Math.cos(stepAngle) + offset;
    other.foot.y =
      offset +
      1.2 * other.stepSize * Math.sin(stepAngle) +
      other.yOff +
      other.height;
    const Ymax = other.yOff + other.height;
    if (other.foot.y > Ymax) other.foot.y = Ymax;

    //calculate knee position as intersection of circle from hip and foot
    const d = Math.sqrt(
      (other.hip.x - other.foot.x) * (other.hip.x - other.foot.x) +
        (other.hip.y - other.foot.y) * (other.hip.y - other.foot.y)
    );
    const l =
      (other.legLength1 * other.legLength1 -
        other.legLength2 * other.legLength2 +
        d * d) /
      (2 * d);
    const h = Math.sqrt(other.legLength1 * m.legLength1 - l * l);
    other.knee.x =
      (l / d) * (other.foot.x - other.hip.x) -
      (h / d) * (other.foot.y - other.hip.y) +
      other.hip.x +
      offset;
    other.knee.y =
      (l / d) * (other.foot.y - other.hip.y) +
      (h / d) * (other.foot.x - other.hip.x) +
      other.hip.y;
  },
  otherDraw() {
    for(let i = 0; i < otherPlayers.length; i++) {
      if(otherPlayers[i].pl.pos.x !== 0 && otherPlayers[i].pl.pos.y !== 0) {
        const ppl = otherPlayers[i].pl;
        // Retrieve the last drawn position or default to the current position
        const lastDrawnX = lastDrawnPositions[i] ? lastDrawnPositions[i].x : ppl.pos.x;
        const lastDrawnY = lastDrawnPositions[i] ? lastDrawnPositions[i].y : ppl.pos.y;

        // Interpolate from the last drawn position
        const interpolatedX = lastDrawnX + (ppl.pos.x - lastDrawnX) * 0.15;
        const interpolatedY = lastDrawnY + (ppl.pos.y - lastDrawnY) * 0.15;

        // Update the last drawn position
        lastDrawnPositions[i] = { x: interpolatedX, y: interpolatedY };
        ctx.fillStyle = m.fillColor;
        ctx.save();
        ctx.translate(interpolatedX, interpolatedY);
        m.otherCalcLeg(Math.PI, -3, otherPlayers[i].pl);
        m.otherDrawLeg("#4a4a4a", otherPlayers[i].pl, i);
        m.otherCalcLeg(0, 0, otherPlayers[i].pl);
        m.otherDrawLeg("#333", otherPlayers[i].pl, i);
        ctx.rotate(otherPlayers[i].pl.angle);
        ctx.beginPath();
  ...
  otherDrawLeg(stroke, other, i) {     
    // Retrieve the last drawn position or default to the current position
    const lastDrawnHipX = lastDrawnHip[i] ? lastDrawnHip[i].x : other.hip.x;
    const lastDrawnHipY = lastDrawnHip[i] ? lastDrawnHip[i].y : other.hip.y;

    // Interpolate from the last drawn position
    const interpolatedHipX = lastDrawnHipX + (other.hip.x - lastDrawnHipX) * 0.9;
    const interpolatedHipY = lastDrawnHipY + (other.hip.y - lastDrawnHipY) * 0.9;

    // Update the last drawn position
    lastDrawnHip[i] = { x: interpolatedHipX, y: interpolatedHipY };
    
    // Retrieve the last drawn position or default to the current position
    const lastDrawnKneeX = lastDrawnKnee[i] ? lastDrawnKnee[i].x : other.knee.x;
    const lastDrawnKneeY = lastDrawnKnee[i] ? lastDrawnKnee[i].y : other.knee.y;

    // Interpolate from the last drawn position
    const interpolatedKneeX = lastDrawnKneeX + (other.knee.x - lastDrawnKneeX) * 0.9;
    const interpolatedKneeY = lastDrawnKneeY + (other.knee.y - lastDrawnKneeY) * 0.9;

    // Update the last drawn position
    lastDrawnKnee[i] = { x: interpolatedKneeX, y: interpolatedKneeY };
    
    // Retrieve the last drawn position or default to the current position
    const lastDrawnFootX = lastDrawnFoot[i] ? lastDrawnFoot[i].x : other.foot.x;
    const lastDrawnFootY = lastDrawnFoot[i] ? lastDrawnFoot[i].y : other.foot.y;

    // Interpolate from the last drawn position
    const interpolatedFootX = lastDrawnFootX + (other.foot.x - lastDrawnFootX) * 0.9;
    const interpolatedFootY = lastDrawnFootY + (other.foot.y - lastDrawnFootY) * 0.9;

    // Update the last drawn position
    lastDrawnFoot[i] = { x: interpolatedFootX, y: interpolatedFootY };
    ctx.save();
    ctx.scale(other.flipLegs, 1); //leg lines
    ctx.beginPath();
    ctx.moveTo(interpolatedHipX, interpolatedHipY);
    ctx.lineTo(interpolatedKneeX, interpolatedKneeY);
    ctx.lineTo(interpolatedFootX, interpolatedFootY);
    ...
    //toe lines
    ctx.beginPath();
    ctx.moveTo(interpolatedFootX, interpolatedFootY);
    ctx.lineTo(interpolatedFootX - 15, interpolatedFootY + 5);
    ctx.moveTo(interpolatedFootX, interpolatedFootY);
    ctx.lineTo(interpolatedFootX + 15, interpolatedFootY + 5);
    ctx.lineWidth = 4;
    ctx.stroke();
    ...
    //hip joint
    ctx.beginPath();
    ctx.arc(interpolatedHipX, interpolatedHipY, 11, 0, 2 * Math.PI);
    //knee joint
    ctx.moveTo(interpolatedKneeX + 7, interpolatedKneeY);
    ctx.arc(interpolatedKneeX, interpolatedKneeY, 7, 0, 2 * Math.PI);
    //foot joint
    ctx.moveTo(interpolatedFootX + 6, interpolatedFootY);
    ctx.arc(interpolatedFootX, interpolatedFootY, 6, 0, 2 * Math.PI);
    ctx.fillStyle = other.fillColor;
    ctx.fill();
    ctx.lineWidth = 2;
    ctx.stroke();
    ctx.restore();
  ...
  setMaxEnergy(isMessage = true) {
    m.maxEnergy =
      ...
      tech.harmonicEnergy +
      2.66 * tech.isGroundState +
      3 * tech.isRelay * tech.isFlipFlopOn * tech.isRelayEnergy +
      1.5 * (m.fieldMode === 1) +
      (m.fieldMode === 0 || m.fieldMode === 1) * 0.05 * m.coupling +
      0.4 * tech.isStandingWaveExpand;
    if (isMessage)
      simulation.makeTextLog(
        `<span class='color-var'>m</span>.<span class='color-f'>maxEnergy</span> <span class='color-symbol'>=</span> ${m.maxEnergy.toFixed(
          2
        )}`
      );
  },
  ...
  setFieldRegen() {
    ...
    if (m.fieldMode === 0 || m.fieldMode === 4)
      m.fieldRegen += 0.000133 * m.coupling; //return `generate <strong>${(6*couple).toFixed(0)}</strong> <strong class='color-f'>energy</strong> per second`
      ...
      if (m.energy < m.minEnergyToDeflect) {
        ...
        if (tech.isLaserField) {
          simulation.ephemera.push({
            ...
            do() {
              m.count--;
              if (m.count < 0) simulation.removeEphemera(m.name);
  ...
  couplingDescription(couple = m.coupling) {
    switch (m.fieldMode) {
      ...
      case 1: //standing wave
        ...
        return `+${(couple * 5).toFixed(
          0
        )} maximum <strong class='color-f'>energy</strong>`;
      case 2: //perfect diamagnetism
        return `<span style = 'font-size:95%;'><strong>deflecting</strong> condenses ${(
          0.1 * couple
        ).toFixed(2)} <strong class='color-s'>ice IX</strong></span>`;
      // return `<span style = 'font-size:89%;'><strong>invulnerable</strong> <strong>+${2*couple}</strong> seconds post collision</span>`
      case 3: //negative mass
        return `<strong>+${(100 * (1 - 0.973 ** couple)).toFixed(
          1
        )}%</strong> <strong class='color-defense'>defense</strong>`;
      case 4: //assembler
        return `generate <strong>${(0.8 * couple).toFixed(
          1
        )}</strong> <strong class='color-f'>energy</strong> per second`;
      case 5: //plasma
        return `<strong>+${(1.5 * couple).toFixed(
          1
        )}%</strong> <strong class='color-d'>damage</strong>`;
      case 6: //time dilation
        return `<strong>+${(5 * couple).toFixed(
          0
        )}%</strong> longer <strong style='letter-spacing: 2px;'>stopped time</strong>`; //<strong>movement</strong>, <strong>jumping</strong>, and
      case 7: //cloaking
        return `<strong>+${(3.3 * couple).toFixed(
          1
        )}%</strong> ambush <strong class='color-d'>damage</strong>`;
      case 8: //pilot wave
        return `<strong>+${(4 * couple).toFixed(
          0
        )}%</strong> <strong class='color-block'>block</strong> collision <strong class='color-d'>damage</strong>`;
      case 9: //wormhole
        return `<span style = 'font-size:89%;'>after eating <strong class='color-block'>blocks</strong> <strong>+${(
          2 * couple
        ).toFixed(0)}</strong> <strong class='color-f'>energy</strong></span>`;
    }
  },
  ...
  setField(index) {
    ...
    simulation.makeTextLog(
      `<span class='color-var'>m</span>.setField("<span class='color-text'>${
  ...
  fieldUpgrades: [
        ...
        m.harmonic3Phase = () => {
          ...
          for (let i = 0, len = mob.length; i < len; ++i) {
            if (
              ...
              // && Matter.Query.ray(map, mob[i].position, m.pos).length === 0
              mob[i].locatePlayer();
              if (m.drainCD > m.cycle) {
                m.pushMass(mob[i], 0);
              } else {
                m.pushMass(mob[i]);
                m.drainCD = m.cycle + 15;
              }
            }
          }
        };
    ...
    },
    {
      ...
      description:
        "<strong>deflecting</strong> does not drain <strong class='color-f'>energy</strong><br>maintains <strong>functionality</strong> while <strong>inactive</strong><br>generate <strong>5</strong> <strong class='color-f'>energy</strong> per second",
      ...
      effect: () => {
            ...
            for (let i = 0, len = mob.length; i < len; ++i) {
              if (
                ...
                Vector.dot(
                  ...
                  Vector.normalise(Vector.sub(mob[i].position, m.fieldPosition))
                ) > m.fieldThreshold &&
                ...
                if (tech.blockDmg) {
                  ...
                  ctx.fillStyle = `rgba(110,150,220, ${
                    0.2 + 0.4 * Math.random()
                  })`;
                  ctx.strokeStyle = "#000";
        ...
        m.hold = function () {
          ...
          if (m.isHolding) {
            ...
            if (m.holdingTarget) {
              ctx.fillStyle = `rgba(110,150,220, ${
                0.06 + 0.03 * Math.random()
              })`;
              ctx.strokeStyle = `rgba(110,150,220, ${
                0.35 + 0.05 * Math.random()
              })`;
            } else {
              ctx.fillStyle = `rgba(110,150,220, ${
                0.27 + 0.2 * Math.random() - 0.1 * wave
              })`;
              ctx.strokeStyle = `rgba(110,150,220, ${
                0.4 + 0.5 * Math.random()
              })`;
            ...
            if (!input.field) {
              ...
              ctx.strokeStyle = `rgba(110,180,255, ${
                0.4 + 0.5 * Math.random()
              })`;
              ctx.beginPath();
              ctx.arc(
    ...
    },
    {
      ...
      description:
        "use <strong class='color-f'>energy</strong> to nullify &nbsp;<strong style='letter-spacing: 7px;'>gravity</strong><br><strong>+55%</strong> <strong class='color-defense'>defense</strong><br>generate <strong>6</strong> <strong class='color-f'>energy</strong> per second",
      fieldDrawRadius: 0,
      effect: () => {
        m.fieldFire = true;
            ...
            if (m.energy > DRAIN) {
              if (tech.isFlyFaster) {
                ...
                if (input.down) {
                  ...
                  m.fieldDrawRadius = m.fieldDrawRadius * 0.97 + 500 * 0.03;
                  moveThis(powerUp, m.fieldDrawRadius, 0);
                  moveThis(body, m.fieldDrawRadius, 0);
                } else if (input.up) {
                  //up
                  m.energy -= 5 * DRAIN;
                  m.fieldDrawRadius = m.fieldDrawRadius * 0.97 + 1100 * 0.03;
                  player.force.y -= 2.25 * player.mass * simulation.g;
                  moveThis(powerUp, m.fieldDrawRadius, 1.8);
                  moveThis(body, m.fieldDrawRadius, 1.8);
                } else {
                  m.energy -= DRAIN;
                  m.fieldDrawRadius = m.fieldDrawRadius * 0.97 + 800 * 0.03;
                  player.force.y -= 1.07 * player.mass * simulation.g; // slow upward drift
                  moveThis(powerUp, m.fieldDrawRadius);
                  moveThis(body, m.fieldDrawRadius);
                }
              } else {
                //look for nearby objects to make zero-g
                ...
                if (input.down) {
                  //down
                  player.force.y -= 0.5 * player.mass * simulation.g;
                  m.fieldDrawRadius = m.fieldDrawRadius * 0.97 + 400 * 0.03;
                  verticalForce(powerUp, m.fieldDrawRadius, 0.7);
                  verticalForce(body, m.fieldDrawRadius, 0.7);
                } else if (input.up) {
                  //up
                  m.energy -= 5 * DRAIN;
                  m.fieldDrawRadius = m.fieldDrawRadius * 0.97 + 850 * 0.03;
                  player.force.y -= 1.45 * player.mass * simulation.g;
                  verticalForce(powerUp, m.fieldDrawRadius, 1.38);
                  verticalForce(body, m.fieldDrawRadius, 1.38);
                } else {
                  m.energy -= DRAIN;
                  m.fieldDrawRadius = m.fieldDrawRadius * 0.97 + 650 * 0.03;
                  player.force.y -= 1.07 * player.mass * simulation.g; // slow upward drift
                  verticalForce(powerUp, m.fieldDrawRadius);
                  verticalForce(body, m.fieldDrawRadius);
                }
              }
              ...
              //     const ICE_DRAIN = 0.0005
              //     for (let i = 0, len = mob.length; i < len; i++) {
              //         if (!mob[i].isMobBullet && !mob[i].shield && !mob[i].isShielded && ((mob[i].distanceToPlayer() + mob[i].radius) < m.fieldDrawRadius)) {
    ...
    },
    {
      ...
      description: `excess <strong class='color-f'>energy</strong> used to build ${
        simulation.molecularMode === 0
          ? "<strong class='color-p' style='letter-spacing: 2px;'>spores"
          : simulation.molecularMode === 1
          ? "<strong>missiles"
          : simulation.molecularMode === 2
          ? "<strong class='color-s'>ice IX"
          : "<strong>drones"
      }</strong><br>use <strong class='color-f'>energy</strong> to <strong>deflect</strong> mobs<br>generate <strong>12</strong> <strong class='color-f'>energy</strong> per second`,
      //   simulation.molecularMode: Math.floor(4 * Math.random()), //0 spores, 1 missile, 2 ice IX, 3 drones
      setDescription() {
        return `excess <strong class='color-f'>energy</strong> used to build ${
          simulation.molecularMode === 0
            ? "<strong class='color-p' style='letter-spacing: 2px;'>spores"
            : simulation.molecularMode === 1
            ? "<strong>missiles"
    ...
    },
    {
      name: "plasma torch",
      description:
        "use <strong class='color-f'>energy</strong> to emit short range <strong class='color-plasma'>plasma</strong><br><strong class='color-d'>damages</strong> and <strong>pushes</strong> mobs away<br>generate <strong>10</strong> <strong class='color-f'>energy</strong> per second",
      set() {
          ...
          m.plasmaBall = Bodies.circle(
            ...
            1,
            {
                ...
                Matter.Body.setPosition(
                  ...
                  Vector.add(
                    ...
                    Vector.mult(
                      ...
                      circleRadiusScale * m.circleRadius
                    )
                  )
                );
              },
              fire() {
                ...
                if (m.circleRadius < 10) m.isPopping = true;
              },
              scale(scale) {
                Matter.Body.scale(m.plasmaBall, scale, scale); //shrink fast
                if (m.circleRadius < m.radiusLimit) m.reset();
              },
              reset() {
                // console.log(m.circleRadius)
              ...
              do() {
                if (m.isOn) {
                  ...
                  // const dmg = m.damage * m.dmgScale
                  // for (let i = 0, len = whom.length; i < len; i++) {
                  //     const mobHit = (who) => {
                  //         if (who.alive) {
                  //             if (!m.isAttached && !who.isMobBullet) m.isPopping = true
                  //             who.damage(dmg);
                  //             // if (who.shield) m.scale(Math.max(0.9, 0.99 - 0.5 / m.plasmaBall.circleRadius))
                  //             if (who.speed > 5) {
                  ...
                  const dmg = m.damage * m.dmgScale;
                  const arcList = [];
                  const damageRadius = circleRadiusScale * m.circleRadius;
                  ...
                  for (let i = 0, len = mob.length; i < len; i++) {
                    if (
                      ...
                      const sub = Vector.magnitude(
                        Vector.sub(m.position, mob[i].position)
                      );
                      if (sub < damageRadius + mob[i].radius) {
                        // if (!m.isAttached && !mob[i].isMobBullet) m.isPopping = true
                        mob[i].damage(dmg);
                        if (mob[i].speed > 5) {
                          Matter.Body.setVelocity(mob[i], {
                            //friction
                        ...
                        Matter.Query.ray(map, mob[i].position, m.position)
                          .length === 0
                      ) {
                  ...
                  for (let i = 0; i < arcList.length; i++) {
                    if (tech.plasmaDischarge > Math.random()) {
                      ...
                      who.damage(dmg * 4);
                      //draw arcs
                      const sub = Vector.sub(who.position, m.position);
                  ...
                  if (m.speed > 10) {
                    const scale = 0.998;
                    Matter.Body.setVelocity(this, {
                      x: scale * m.velocity.x,
                      y: scale * m.velocity.y,
                    });
                  }
                  ...
                  const alpha = m.alpha + 0.1 * Math.random();
                  ...
                  ctx.beginPath();
                  ctx.arc(m.position.x, m.position.y, radius, 0, 2 * Math.PI);
                  ctx.fill();
                  //draw arcs
            ...
            if (m.isHolding) {
              ...
              if (!m.plasmaBall.isAttached) {
                ...
                if (tech.isCapacitor) {
                  ...
                  const scale =
                    1 +
                    48 * Math.pow(Math.max(1, m.plasmaBall.circleRadius), -1.8);
                  Matter.Body.scale(m.plasmaBall, scale, scale); //grow
                } else {
                  m.energy -= m.plasmaBall.drain;
                  const scale =
                    1 +
                    16 * Math.pow(Math.max(1, m.plasmaBall.circleRadius), -1.8);
                  Matter.Body.scale(m.plasmaBall, scale, scale); //grow
                }
                if (m.energy > m.maxEnergy) {
                  m.energy -= m.plasmaBall.drain * 2;
    ...
    },
    {
      name: "time dilation",
      description:
        "use <strong class='color-f'>energy</strong> to <strong style='letter-spacing: 2px;'>stop time</strong><br><strong>+20%</strong> movement and <strong><em>fire rate</em></strong><br>generate <strong>12</strong> <strong class='color-f'>energy</strong> per second",
      set() {
          ...
          m.hold = function () {
            ...
            if (m.isHolding) {
              ...
              if (!m.holdingTarget) {
                ...
                m.rewindCount += 6;
                const DRAIN = 0.003;
                let history = m.history[(m.cycle - m.rewindCount) % 600];
                if (m.rewindCount > 599 || m.energy < DRAIN) {
                  ...
                  if (tech.isRewindBot && !(m.rewindCount % 60)) {
                  ...
                  if (tech.isRewindGrenade && !(m.rewindCount % 30)) {
                    b.grenade(m.pos, m.rewindCount); //Math.PI / 2
                    const who = bullet[bullet.length - 1];
      ...
      effect: () => {
        ...
        m.hold = function () {
          ...
          if (m.energy < 0.05 && m.fireCDcycle < m.cycle && !input.fire)
            m.fireCDcycle = m.cycle;
          if (m.fireCDcycle + 30 < m.cycle && !input.fire) {
              ...
              if (m.energy > drain) {
                for (let i = 0, len = mob.length; i < len; ++i) {
                  if (
                    Vector.magnitude(Vector.sub(mob[i].position, m.pos)) <
                      stunRange &&
                    Matter.Query.ray(map, mob[i].position, m.pos).length ===
                      0 &&
                    !mob[i].isBadTarget
                  ) {
                    isMobsAround = true;
          ...
          if (
            m.isSneakAttack &&
            m.sneakAttackCycle +
              Math.min(120, 0.5 * (m.cycle - m.enterCloakCycle)) >
              m.cycle
          ) {
    ...
    },
    {
          ...
          if (m.fieldCDcycle < m.cycle) {
            ...
            if (input.field) {
              ...
              if (
                ...
                m.drain = (0.06 + 0.006 * Math.sqrt(mag)) * 2;
              } else {
                m.drain = tech.isFreeWormHole
                  ? 0
                  : 0.06 + 0.006 * Math.sqrt(mag);
              }
      ...
      //         if (input.field && m.fieldCDcycle < m.cycle) { //not hold but field button is pressed
      //             const DRAIN = 0.01
      //             if (m.rewindCount < 289 && m.energy > DRAIN) {
      //                 m.energy -= DRAIN
      ...
      //                     ctx.fillRect(0, 0, canvas.width, canvas.height);
      //                 }
      //                 let history = m.history[(m.cycle - m.rewindCount) % 300]
      //                 Matter.Body.setPosition(player, history.position);
...
const m2 = {
  ...
  spawn(playerId) {
    let vertices = Vertices.fromPath("0,40, 50,40, 50,115, 30,130, 20,130, 0,115, 0,40");
    let pB = Bodies.fromVertices(0, 0, vertices);

    let jS = Bodies.rectangle(0, 46, 36, 6, {
      sleepThreshold: 99999999999,
      isSensor: true,
    });

    vertices = Vertices.fromPath("16 -82  2 -66  2 -37  43 -37  43 -66  30 -82");
    let h = Bodies.fromVertices(0, -55, vertices);

    let hS = Bodies.rectangle(0, -57, 48, 45, {
      sleepThreshold: 99999999999,
      isSensor: true,
    });

    let playerBody = Body.create({
      parts: [pB, h, jS, hS],
      inertia: Infinity,
      friction: 0.002,
      frictionAir: 0.001,
    ...
    Matter.Body.setMass(playerBody, m.mass);

    this.playerBodies[playerId] = playerBody;

    Composite.add(engine.world, [playerBody]);
  },
  updatePosition(playerId, pos, yOff, crouch) {
    const playerBody = this.playerBodies[playerId];
    if (playerBody) {
      Matter.Body.setPosition(playerBody, { x: pos.x, y: pos.y + 24.2859});
      Matter.Body.setVelocity(playerBody, { x: 0, y: 0}); // Otherwise it looks goofy
    }
  },
  deleteBody(playerId) {
    const playerBody = this.playerBodies[playerId];
    if (playerBody) {
      Composite.remove(engine.world, [playerBody]);
    }
  }
};